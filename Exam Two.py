...
# Open the file, read each line, split, convert price, and append to properties list
...
# Print the headings and then print the two-dimensional properties list
print(f"{'Address':<30} {'City':<15} {'State':<7} {'Zip Code':<10} {'Price':>12}")
print("-" * 80)
for prop in properties:
    print(f"{prop[0]:<30} {prop[1]:<15} {prop[2]:<7} {prop[3]:<10} ${prop[4]:>11,.2f}")

# Create an empty two-dimensional list zipcodes
zipcodes = []

# Loop through all rows in properties
for prop in properties:
    zipcode = prop[3]
    price = prop[4]
    
    # Loop through rows in zipcodes to find if the zipcode already exists
    found = False
    for z in zipcodes:
        if z[0] == zipcode:
            # Zipcode match found
    ...
    # If no match in zipcodes list, append new row
    if not found:
...
# Loop through zipcodes and print Zipcode, Count, and Average Price
for z in zipcodes:
    zipcode = z[0]
    count = z[1]
    sum_prices = z[2]
